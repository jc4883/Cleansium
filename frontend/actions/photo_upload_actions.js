...
export const fetchPhotoUploads = (dayId) => dispatch => {
  return API_Photo_Upload_Util.fetchPhotoUploads(dayId)
    .then(photoUploads => {
      return dispatch(receivePhotoUploads(photoUploads));
    })
}

export const createPhotoUpload = (dayId, formData) => dispatch => {
  return API_Photo_Upload_Util.createPhotoUpload(dayId, formData)
    .then(photoUpload => {
      return dispatch(receivePhotoUpload(photoUpload));
    })
}
